...
class OptCarlini_GAN:
    ...
    def __init__(self, model, target=True, c=1, lr=0.01, init_scl=0.1,
                 loss_op=0, k=5, use_mask=True, decay=True):
        """
        ...
        if self.use_mask:
            self.m = K.placeholder(dtype='float32', shape=(1, self.latent_dim))
        ...
        model_output = self.model([self.x_in, tf.argmax(self.y, axis=1)])
    ...
    def optimize(self, x, y, weights_path, n_step=1000, prog=True, mask=None):
        ...
        with tf.Session() as sess:
            ...
            x_ = np.copy(x).reshape((1, self.latent_dim))
    ...
    def optimize_search(self, x, y, weights_path, n_step=1000, search_step=10,
        ...
        for c_step in range(search_step):
            ...
            # Evaluate result
            y_pred = self.model.predict(x_adv.reshape((1, self.latent_dim)))[0]
