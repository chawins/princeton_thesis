    ...
    for epoch in range(n_epochs):
        ...
        for index in range(n_batch):
            ...
            for _ in range(N_DIS):
                ...
                z = np.random.normal(0, 0.5, (batch_size, latent_dim))
            ...
            z = np.random.uniform(0, 0.5, (2 * batch_size, latent_dim))
        ...
        # ---------------- Test discriminator -------------------------------- #
        z = np.random.uniform(0, 0.5, (n_test, latent_dim))
        ...
        # ---------------- Test generator ------------------------------------ #
        z = np.random.uniform(0, 0.5, (2 * n_test, latent_dim))
        ...
        g.save_weights("{}weight_g_epoch_{:03d}.hdf5".format(
            save_path, epoch), True)
        d.save_weights("{}weight_d_epoch_{:03d}.hdf5".format(
            save_path, epoch), True)

        # generate some digits to display
        noise = np.random.uniform(0, 0.5, (100, latent_dim))
        ...
        Image.fromarray(img).save(
            '{}plot_epoch_{:03d}_generated.png'.format(vis_path, epoch))
